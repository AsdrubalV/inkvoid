...
export default async function HomePage() {
  ...
  if (!user) {
    // Traer historias más leídas de la semana
    ...
    // Fallback: si no hay vistas aún, traer las más recientes
      ...
      <div className="space-y-16 pb-16">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-gray-800 to-black px-8 py-16 text-white text-center space-y-6">
          ...
          <div className="relative z-10 space-y-4 max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
              <Link
                href="/signup"
                className="rounded-full bg-white px-8 py-3 text-sm font-semibold text-black hover:bg-gray-100 transition"
              >
                Unirme gratis
              </Link>
              <Link
                href="/login"
                className="rounded-full border border-white/30 px-8 py-3 text-sm font-medium text-white hover:bg-white/10 transition"
              >
                Ya tengo cuenta
              </Link>
            </div>
        ...
        {featuredStories.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Link href="/signup" className="text-xs text-gray-500 hover:text-black">
                Ver todo →
              </Link>
            </div>
                ...
                >
                  <div className="relative aspect-[2/3] w-full overflow-hidden rounded-lg border border-border bg-gray-100">
                    {story.cover_url ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center text-[10px] text-gray-400 p-2 text-center">
                        {story.title}
                      </div>
                    )}
                    {/* Overlay al hover */}
                  ...
                  {story.category && (
                    <span className="inline-block rounded-full bg-gray-100 px-1.5 py-0.5 text-[9px] text-gray-500">
                      {story.category}
                    </span>
                  )}
                </Link>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl bg-gradient-to-br from-gray-900 to-black p-6 text-white space-y-3">
            ...
            <p className="text-sm text-gray-300 leading-relaxed">
              Publica tu novela, construye tu audiencia y monetiza tu trabajo.
              Soportamos todos los géneros — desde fantasía épica hasta fanfiction y romance.
            </p>
            <Link href="/signup" className="inline-block rounded-full bg-white px-5 py-2 text-xs font-semibold text-black hover:bg-gray-100 transition">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-gray-100 to-gray-200 p-6 space-y-3">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">
              Descubre miles de historias únicas escritas por autores independientes.
              Comenta, dale like y sigue a tus autores favoritos.
            </p>
            <Link href="/signup" className="inline-block rounded-full bg-black px-5 py-2 text-xs font-semibold text-white hover:bg-gray-800 transition">
              Explorar historias
            </Link>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              "Fantasy", "Dark Fantasy", "Romance", "Fanfiction", "LitRPG",
              "Isekai", "Thriller", "Horror", "SciFi", "Supernatural",
              "Adventure", "Mystery", "Dystopian", "Erotic", "Mythology"
            ].map((genre) => (
              <Link
                key={genre}
                href="/signup"
                className="rounded-full border border-border px-4 py-1.5 text-sm text-gray-700 hover:bg-black hover:text-white hover:border-black transition"
              >
                {genre}
              </Link>
            ))}
        ...
        <div className="rounded-2xl bg-black text-white text-center py-12 px-6 space-y-4">
          ...
          <p className="text-gray-400 text-sm max-w-md mx-auto">
            Únete a InkVoid gratis. Lee, escribe y conecta con una comunidad apasionada por las historias.
          </p>
          <Link
            href="/signup"
            className="inline-block rounded-full bg-white px-8 py-3 text-sm font-semibold text-black hover:bg-gray-100 transition"
          >
